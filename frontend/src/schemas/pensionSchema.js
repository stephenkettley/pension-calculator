...
const numberField = (name) =>
  z
    ...
    .pipe(z.number().finite(`${name} is required.`));

export const pensionSchema = z
  .object({
    currentAge: numberField("Current age").pipe(
      z
        .number()
    ...
    retirementAge: numberField("Retirement age").pipe(
      z
        .number()
    ...
    currentBalance: numberField("Current balance").pipe(
      z
        .number()
    ...
    pensionValue: numberField("Pension value").pipe(
      z
        .number()
        .int("Pension value must be a whole number.")
        .min(0, "Pension value cannot be negative.")
    ),

    annualGrowthRate: numberField("Annual growth rate").pipe(
      z
        .number()
